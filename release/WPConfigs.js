...
//   enforce           : true,
// },
}}},this.plugins=[new _webpack["default"].DefinePlugin({env:JSON.stringify(j),"service.env":JSON.stringify(j),"process.env.NODE_ENV":JSON.stringify(j)}),new _miniCssExtractPlugin["default"]({filename:"styles/[name].css"}),// new I18nPlugin(languageConfig, optionsObj),
...
templateParameters:{compile_date:new Date().toLocaleString("zh",{hour12:!1})},filename:"index.html",hash:!1,meta:{viewport:"width=device-width, initial-scale=1, user-scalable=no"},minify:{collapseWhitespace:!0,removeComments:!1,removeRedundantAttributes:!0,removeScriptTypeAttributes:!0,removeStyleLinkTypeAttributes:!0,useShortDoctype:!0}})],this.output=function(){var a=p.output||{path:"dist"};return"string"==typeof a&&(a={path:a}),_objectSpread({hashSalt:"Oulate X",publicPath:""},a,{// path         : builtMode === MODE_DEVELOPMENT ? '/var/tmp/@xrosy/generator' : path.join(absWorkspace, userOutput.path),
path:_path["default"].join(k,a.path),filename:"js/[name].js",chunkFilename:"js/[name].js"})}(),this.optimization.noEmitOnErrors=!0,q===_constant.MODE_DEVELOPMENT&&(this.plugins.unshift(new _cleanWebpackPlugin["default"]({cleanStaleWebpackAssets:!0,cleanOnceBeforeBuildPatterns:[this.output.path]})),this.plugins.push(new _webpack["default"].optimize.OccurrenceOrderPlugin,new _webpack["default"].HotModuleReplacementPlugin,new _webpack["default"].NoEmitOnErrorsPlugin))}/*
    initEntries() {
      const userApps = apps;
    ...
    */;// develop service
return q===_constant.MODE_DEVELOPMENT?(0,_WPServer["default"])((0,_webpack["default"])(_objectSpread({},t,{output:_objectSpread({},t.output,{publicPath:"/"})})),g):void// build
(0,_webpack["default"])(t).run(function(){// const { errors } = stats.toJson();
